        ...
        <tr>
            ...
            <td>
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                    <span style="font-family: monospace; font-size: 1.2em; line-height: 1;">••••••••</span>
                    <button class="action-btn" onclick="window.adminMembres.togglePassword(this, '${escapeHtml(m.password || '')}')" title="Voir le mot de passe" style="padding: 2px;">
...
window.adminMembres = {
    ...
    togglePassword(btn, password) {
        const display = btn.previousElementSibling;
        const icon = btn.querySelector('i'); // Assumons que le bouton contient une balise <i> ou est stylisé

        // Si RemixIcon n'est pas utilisé dans le bouton original (c'était un emoji), on utilise l'emoji
        const isEmoji = !icon;

        if (display.textContent === '••••••••') {
            display.textContent = password || 'Crypté/Inaccessible';
            display.style.color = password ? 'var(--color-primary)' : 'var(--color-text-muted)';
            if (isEmoji) btn.textContent = '🙈';
            else icon.className = 'ri-eye-off-line';

            // Masquer auto après 5s
            setTimeout(() => {
                if (display.textContent !== '••••••••') {
                    display.textContent = '••••••••';
                    display.style.color = '';
                    if (isEmoji) btn.textContent = '👁️';
                    else icon.className = 'ri-eye-line';
                }
            }, 5000);
        } else {
            display.textContent = '••••••••';
    ...
    async editMember(id) {
        try {
            ...
            // On ne remplit pas le mot de passe pour des raisons de sécurité lors de l'édition
    ...
    async handleMemberSubmit(e) {
        ...
        if (password) data.password = password; // Envoi du mot de passe si fourni
