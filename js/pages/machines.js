...
import { injectPageKeywords } from '../seo-keywords.js';
...
function createMachineCard(machine) {
    ...
    const safeName = sanitizeHTML(machine.name);
    const safeCategory = sanitizeHTML(machine.category);
    const safePrice = sanitizeHTML(machine.priceRange || 'Sur devis');
    const safeStatus = sanitizeHTML(machine.status);
    ...
    div.innerHTML = `
        ...
        <div class="p-6 flex-grow flex flex-col">
            <h3 class="text-xl font-bold text-gray-900 mb-3 group-hover:text-red-700 transition-colors">${safeName}</h3>
            
            ${machine.specs && machine.specs.length > 0 ? `
                <div class="mb-6 space-y-2">
                    ${machine.specs.slice(0, 3).map(spec => `
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">${sanitizeHTML(spec.label)}</span>
                            <span class="font-medium text-gray-900">${sanitizeHTML(spec.value)}</span>
                        </div>
                    `).join('')}
                </div>
            ` : ''}
            ...
            <div class="mt-auto pt-4 border-t border-gray-100">
                <button class="w-full bg-gray-900 text-white py-3 px-4 rounded-lg font-bold uppercase text-sm tracking-wide hover:bg-red-700 transition-colors flex items-center justify-center gap-2 group btn-reserve" data-id="${machine.id}" data-name="${safeName}">
                    Réserver
...
function createReservationModal() {
    ...
    document.getElementById('reservation-form').addEventListener('submit', async (e) => {
        ...
        const data = {
            machine_id: document.getElementById('reservation-machine-id').value,
            machine_name: document.getElementById('reservation-machine-name').value,
            user_name: document.getElementById('reservation-name').value,
            user_email: document.getElementById('reservation-email').value,
            user_phone: document.getElementById('reservation-phone').value,
