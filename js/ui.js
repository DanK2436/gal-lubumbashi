...
export function createMachineCard(machine) {
    ...
    return createCard({
        title: machine.name,
        image: machine.image,
        description: specs ? `<ul class="list-none ml-0 text-sm">${specs}</ul>` : '',
        badge,
        meta: machine.priceRange ? `<span class="text-primary font-semibold">${machine.priceRange}</span>` : '',
        actions: [
            `<button class="btn btn--whatsapp btn--sm" data-whatsapp data-whatsapp-message="${getMachineWhatsAppMessage(machine)}">
        ${t('machines.interested')}
      </button>`
...
export function createFormationCard(formation) {
    ...
    return createCard({
        ...
        badge: `<span class="badge badge--primary">${formation.theme}</span>`,
        meta: `<span>📚 ${duration}</span>`,
        actions: [
            `<button class="btn btn--primary btn--sm" data-formation-id="${formation.id}">
        ${t('formations.start')}
